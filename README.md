# 🕒 ChronoHub - Clock, Timer & Stopwatch Application & World Clock & Alarm 

Bu React va Vite yordamida yaratilayotgan, jamoaviy open-source loyiha. Loyihada real vaqtni ko'rsatuvchi soat, aniq ishlovchi sekundomer (Stopwatch) va taymer funksiyalari mavjud.

---

## 🚀 Loyihani kompyuterda ishga tushirish

Loyihani o'z noutbugingizga yuklab olib, ishga tushirish uchun quyidagi buyruqlarni ketma-ket terminalda bajaring:

### 1. Vercel orqaliy foydalanib ko'ring ©.
----- https://clocktime-pi.vercel.app/  ----

### 2. Loyihani klon qilish (Yuklab olish):
```bash
git clone https://github.com/asadekk/Digital-Clock.git


